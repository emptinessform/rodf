...
## 대기열 (우선순위순)
- [ ] **커버리지 기능 구현** — 와일드 UNSUPPORTED 16건을 빈도순으로:
  note(각주) → list(목록) → table(표) → frame/shape → 인덱스류 → forms.
  각 기능 착수 시 해당 요소를 unsupported_kind()에서 빼고 렌더 경로 구현.
- [ ] **Docker 오라클에서 기본 탭 2cm 로케일 검증** — LO 프로그램 기본 탭
  간격 2cm는 이 머신(한국어 Windows) 실측. C 로케일 Docker LO에서 다를
  가능성 → 주간 오라클 또는 수동 실행으로 space.odt 점수 확인.
...
- [ ] **M2.5 공개 대시보드** — "Are we ODF yet?" 스코어보드 공개 페이지.
