# FSL(Font Simulation Library) 라이선스 분석

> rodf의 핵심 방향인 FSL — 문서에 쓰인 폰트가 시스템에 없어도 유사한 모양을
...
- **한국**: 대법원은 서체 도안 자체의 저작물성을 부정한다 — 미적 요소가
  ...
  저작물이다 (대법원 2001. 6. 29. 선고 99다23246 판결). FSL은 파일을 복제하지
  않으므로 침해가 성립할 대상이 없다.
...
FSL은 외형을 자유 라이선스 폰트에서 가져오고 원본 외형을 복제하지 않는다.
...
키로 참조하는 것(지칭적 사용)은 일반적으로 허용된다. 런타임 FSL은 파일을
...
| 2 | OFL Reserved Font Name — OFL 폰트를 수정(메트릭 교체)해 배포하면 원래 이름 사용 금지 | OFL 조항 | 산출물 개명(mcfg 산출물 관행과 동일). 런타임 FSL은 파일을 수정하지 않으므로 해당 없음 |
...
rodf's FSL (Font Simulation Library) substitutes missing document fonts with
